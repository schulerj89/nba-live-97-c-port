...
namespace nba97 {
std::string captureGameDrawEnvironment() {
 DrawComposed c;
 if(c.run()!=NBA97_TEXT_COMPLETE||!c.progress.completed||c.draws.size()!=2||c.gpu.completions!=5||c.videoBinding.completions!=2||c.copies!=4)
   throw std::runtime_error("draw environment scene composition failed");
 const bool cacheEqual=std::memcmp(c.bytes.data()+c.at(0x800c55d0u),c.bytes.data()+c.at(0x80021eecu),0x5c)==0;
 check(cacheEqual&&c.packets==2&&c.submits==2&&c.drawCopies==2);
 std::ostringstream o;o<<"{\"program\":\"GAMEONLY\",\"address\":\"0x80099ACC\",\"inclusive_end\":\"0x80099B8F\",\"bytes\":196,\"instructions\":49,"
  "\"classification\":\"no direct visual effect\",\"scope\":\"actual scene, display, draw, video query, GPU command and BIOS trampoline; synthetic packet, submission, MMIO and BIOS services\","
  "\"completed\":true,\"parent_completed\":true,\"draw_calls\":"<<c.draws.size()<<",\"packet_calls\":"<<c.packets<<",\"submit_calls\":"<<c.submits<<",\"draw_copy_calls\":"<<c.drawCopies<<",\"all_copy_calls\":"<<c.copies
  <<",\"cache_matches_last_environment\":"<<(cacheEqual?"true":"false")<<",\"tags_before\":[301989888,872415232],\"tags_after\":[";
 for(unsigned i=0;i<2;++i){if(i)o<<',';const U32 a=i?0x80021f64u:0x80021f08u;U32 tag=0;for(unsigned b=0;b<4;++b)tag|=U32(c.bytes[c.at(a+b)])<<(8*b);o<<tag;}
 o<<"],\"draws\":[";
 for(size_t i=0;i<c.draws.size();++i){if(i)o<<',';const auto&q=c.draws[i];
   o<<"{\"call_pc\":"<<c.drawEvents[i].pc<<",\"operations\":"<<q.operations<<",\"reads\":"<<q.reads<<",\"stores\":"<<q.stores<<",\"callbacks\":"<<q.callbacks_completed<<",\"return_v0\":"<<q.machine.registers.gpr[2].word<<",\"return_address\":"<<q.machine.registers.gpr[31].word<<",\"hi\":"<<q.machine.hi.word<<",\"copy_t1\":"<<q.machine.registers.gpr[9].word<<",\"copy_t2\":"<<q.machine.registers.gpr[10].word<<"}";}
 o<<"]}";return o.str();
}
}
