...
static int allocation(Run* r,uint32_t name,uint32_t size,uint32_t* style){
    ...
    e->argument[0]=name;e->argument[1]=size;e->argument[2]=0x20;e->argument[3]=1;
...
int nba97_game_text_pools(Nba97GameTextPoolContext* context,const Nba97GameTextPoolArguments* arguments,
    ...
    int32_t ids,packets;size_t i,j;
    ...
    TRY(allocation(r,args.name,size,&style));base=style+0x58u;
