#include "game_draw_area_start.h"

#include <string.h>

typedef Nba97GameDrawAreaStartWord Word;
...
static int valid_machine(const Nba97GameDrawAreaStartMachine *machine) {
  unsigned index;
...
static void journal_read(Run *run, uint32_t pc, uint32_t address,
  ...
  if (index < run->context->access_journal_capacity) {
    Nba97GameDrawAreaStartAccess *event = &run->context->access_journal[index];
    event->pc = pc;
...
int nba97_game_draw_area_start(Nba97GameDrawAreaStartContext *context,
                               Nba97GameDrawAreaStartProgress *progress) {
  Run storage;
  Run *run = &storage;
  ...
  /* 0x8009A644..0x8009A684: sign-extend x. A negative x clears v0 in the
  ...
  if (condition == DECISION_UNKNOWN) {
    stop(run, UINT32_C(0x8009a64c), 0u);
  ...
  if (condition == DECISION_FALSE) {
    ...
    status = read_fixed(run, UINT32_C(0x8009a65c), UINT32_C(0x800c55c4), 2u,
    ...
    if (condition == DECISION_UNKNOWN) {
      stop(run, UINT32_C(0x8009a674), 0u);
      return NBA97_TEXT_UNKNOWN;
    }
    if (condition == DECISION_FALSE) {
  ...
  /* 0x8009A688..0x8009A6C4: retain the explicit shifted v0, sign-extend y
  ...
  if (condition == DECISION_UNKNOWN) {
    stop(run, UINT32_C(0x8009a690), 0u);
    return NBA97_TEXT_UNKNOWN;
  }
  if (condition == DECISION_FALSE) {
    R(NBA97_MATCH_INITIALIZE_V0) = known_word(UINT32_C(0x800c0000));
    R(NBA97_MATCH_INITIALIZE_V0) = known_word(UINT32_C(0x800c55c6));
    status = read_fixed(run, UINT32_C(0x8009a6a0), UINT32_C(0x800c55c6), 2u,
                        &R(NBA97_MATCH_INITIALIZE_V0));
    if (status != NBA97_TEXT_COMPLETE)
      return status;
    R(NBA97_MATCH_INITIALIZE_V0) =
    ...
    condition = zero_decision(R(NBA97_MATCH_INITIALIZE_V0));
    if (condition == DECISION_UNKNOWN) {
      stop(run, UINT32_C(0x8009a6b8), 0u);
      return NBA97_TEXT_UNKNOWN;
    }
    if (condition == DECISION_FALSE)
  ...
  /* 0x8009A6C8..0x8009A6FC: type bytes one and two select 12-bit fields;
  ...
  status = read_fixed(run, UINT32_C(0x8009a6d0), UINT32_C(0x800c55c0), 1u,
                      &R(NBA97_MATCH_INITIALIZE_V0));
  if (status != NBA97_TEXT_COMPLETE)
    return status;
  R(NBA97_MATCH_INITIALIZE_V0) =
  ...
  if (condition == DECISION_UNKNOWN) {
    stop(run, UINT32_C(0x8009a6e0), 0u);
    return NBA97_TEXT_UNKNOWN;
  }
  if (condition == DECISION_FALSE) {
    R(NBA97_MATCH_INITIALIZE_V1) =
  ...
  /* 0x8009A700..0x8009A70C: form E3, then execute the final OR in JR's
   * delay slot before requiring a known live return address. */
  R(NBA97_MATCH_INITIALIZE_A0) = known_word(UINT32_C(0xe3000000));
  R(NBA97_MATCH_INITIALIZE_V0) =
  ...
  if (R(NBA97_MATCH_INITIALIZE_RA).known_mask != 15u) {
    stop(run, UINT32_C(0x8009a708), 0u);
    return NBA97_TEXT_UNKNOWN;
  }
