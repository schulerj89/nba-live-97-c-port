...
void copy_to_owner(const Nba97GameDrawPacketMachine &source,
                   Nba97GameDrawAreaStartMachine &destination) {
  for (unsigned index = 0u; index != 32u; ++index) {
...
void copy_from_owner(const Nba97GameDrawAreaStartMachine &source,
...
extern "C" void nba97_game_draw_area_start_packet_binding_init(
    Nba97GameDrawAreaStartPacketBinding *binding, size_t operation_budget,
...
extern "C" int
nba97_game_draw_area_start_from_packet(void *opaque,
                                       const Nba97GameTextMemory *memory,
                                       const Nba97GameDrawPacketEvent *event,
                                       Nba97GameDrawPacketMachine *machine) {
  auto *binding = static_cast<Nba97GameDrawAreaStartPacketBinding *>(opaque);
  if (binding == nullptr || memory == nullptr || event == nullptr ||
      machine == nullptr)
    return 0;
  const bool target_kind = event->kind == NBA97_GAME_DRAW_PACKET_CHILD_8009A644;
  const bool target_entry = event->entry == UINT32_C(0x8009a644);
  ...
  binding->result = NBA97_TEXT_ARGUMENT;
  if (!target_kind || !target_entry || event->pc != UINT32_C(0x8009a364) ||
  ...
  ++binding->invocations;
  binding->event = *event;
  binding->result = nba97_game_draw_area_start(&context, &binding->progress);
