...
namespace nba97 {
std::string captureGameDrawModeCommand() {
 ModeComposed c;
 if(c.run()!=NBA97_TEXT_COMPLETE||!c.progress.completed||c.modes.size()!=2||c.dmaRuns.size()!=2||c.submissions.size()!=2||c.packetRuns.size()!=2||c.copies!=4||c.submits!=2)
   throw std::runtime_error("draw-mode scene composition failed");
 check(c.get(0x80021f18u)==0xe1000634u&&c.get(0x80021f74u)==0xe1000634u);
 ...
 std::ostringstream o;o<<"{\"program\":\"GAMEONLY\",\"address\":\"0x8009A5E8\",\"inclusive_end\":\"0x8009A643\",\"bytes\":92,\"instructions\":23,"
  "\"classification\":\"no direct visual effect\",\"scope\":\"actual scene/display/draw/packet/start/end/offset/mode/video/GPU-command/BIOS-trampoline/graphics-submit/packet-DMA owners; one synthetic packet helper, scheduler/critical services and mapped device registers without GPU consumption\","
  "\"completed\":true,\"parent_completed\":true,\"mode_calls\":"<<c.modes.size()<<",\"packet_calls\":"<<c.packetRuns.size()<<",\"dma_calls\":"<<c.dmaRuns.size()<<",\"submit_calls\":"<<c.submits<<",\"copy_calls\":"<<c.copies
  <<",\"cache_matches_last_environment\":"<<(cacheEqual?"true":"false")<<",\"commands\":["<<c.get(0x80021f18u)<<','<<c.get(0x80021f74u)<<"],\"modes\":[";
 for(size_t i=0;i<c.modes.size();++i){if(i)o<<',';const auto&q=c.modes[i];
   o<<"{\"operations\":"<<q.operations<<",\"reads\":"<<q.reads<<",\"return_v0\":"<<q.machine.registers.gpr[2].word<<",\"return_address\":"<<q.machine.registers.gpr[31].word<<",\"hi\":"<<q.machine.hi.word<<"}";}
 o<<"]}";return o.str();
}
}
