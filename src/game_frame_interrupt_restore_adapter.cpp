...
int callIndex(const Nba97MatchFrameCall* call) {
    if (!call)
        return -1;
    switch (call->pc) {
    case UINT32_C(0x8004909c):
        return NBA97_GAME_FRAME_INTERRUPT_RESTORE_CALL_4909C;
    case UINT32_C(0x800491d8):
        return NBA97_GAME_FRAME_INTERRUPT_RESTORE_CALL_491D8;
    case UINT32_C(0x8004926c):
        return NBA97_GAME_FRAME_INTERRUPT_RESTORE_CALL_4926C;
    case UINT32_C(0x800492c0):
        return NBA97_GAME_FRAME_INTERRUPT_RESTORE_CALL_492C0;
    default:
        return -1;
    }
}

bool targetsRestore(const Nba97MatchFrameCall* call) {
    return call && (call->entry == UINT32_C(0x8004900c) ||
        callIndex(call) >= 0);
}
...
void resetTelemetry(Nba97GameFrameInterruptRestoreBinding* binding) {
...
int ioDispatch(void* opaque, const Nba97MatchFrameCall* call,
    ...
    auto& run = *static_cast<AdapterRun*>(opaque);
    if (targetsRestore(call))
        return nba97_game_frame_interrupt_restore_from_match_frame(
...
int nba97_game_frame_interrupt_restore_from_match_frame(void* opaque,
    const Nba97MatchFrameCall* call, Nba97GamePeriodValue* value) {
    auto* binding =
        static_cast<Nba97GameFrameInterruptRestoreBinding*>(opaque);
    const int index = callIndex(call);
    if (!binding || !call || !value || index < 0 ||
        call->entry != UINT32_C(0x8004900c) || call->args[1] != 0 ||
        binding->cp0_status.known_mask > 0x0fu ||
        (!binding->journal && binding->journal_capacity))
        return NBA97_BODY_ARGUMENT;

    Nba97GameFrameInterruptRestoreMachine entry{};
    entry.cp0_status = binding->cp0_status;
    entry.registers.gpr[NBA97_MATCH_INITIALIZE_ZERO] = {0, 0x0f};
    entry.registers.gpr[NBA97_MATCH_INITIALIZE_A0] = {call->args[0], 0x0f};
    ...
    binding->result[index] = nba97_game_frame_interrupt_restore(
