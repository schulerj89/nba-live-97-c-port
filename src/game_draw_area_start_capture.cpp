...
namespace nba97 {
std::string captureGameDrawAreaStart() {
 AreaStartComposed c;
 if(c.run()!=NBA97_TEXT_COMPLETE||!c.progress.completed||c.areas.size()!=2||c.packetRuns.size()!=2||c.copies!=4||c.submits!=2)
   throw std::runtime_error("draw-area start scene composition failed");
 check(c.get(0x80021f0cu)==0xe3008040u&&c.get(0x80021f68u)==0xe3008040u);
 const bool cacheEqual=std::memcmp(c.bytes.data()+c.at(0x800c55d0u),c.bytes.data()+c.at(0x80021eecu),0x5c)==0;check(cacheEqual);
 std::ostringstream o;o<<"{\"program\":\"GAMEONLY\",\"address\":\"0x8009A644\",\"inclusive_end\":\"0x8009A70F\",\"bytes\":204,\"instructions\":51,"
  "\"classification\":\"no direct visual effect\",\"scope\":\"actual scene, display, draw, packet, draw-area start, video query, GPU command and BIOS trampoline; four synthetic packet-word helpers and submission/MMIO/BIOS services\","
  "\"completed\":true,\"parent_completed\":true,\"area_calls\":"<<c.areas.size()<<",\"packet_calls\":"<<c.packetRuns.size()<<",\"submit_calls\":"<<c.submits<<",\"copy_calls\":"<<c.copies
  <<",\"cache_matches_last_environment\":"<<(cacheEqual?"true":"false")<<",\"commands\":["<<c.get(0x80021f0cu)<<','<<c.get(0x80021f68u)<<"],\"areas\":[";
 for(size_t i=0;i<c.areas.size();++i){if(i)o<<',';const auto&q=c.areas[i];
