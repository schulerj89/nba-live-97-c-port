...
namespace {
bool valid_machine(const Nba97GameActorTimersMachine &machine) {
...
extern "C" int
nba97_game_actor_timers_from_match_tick(void *opaque,
                                        const Nba97MatchTickCall *call,
                                        Nba97GamePeriodValue *result) {
  auto *binding = static_cast<Nba97GameActorTimersBinding *>(opaque);
  if (binding == nullptr || call == nullptr || result != nullptr ||
      call->pc != UINT32_C(0x80068e38) || call->entry != UINT32_C(0x8006830c) ||
  ...
  ++binding->invocations;
  binding->event = *call;
  binding->result = nba97_game_actor_timers(&context, &binding->progress);
