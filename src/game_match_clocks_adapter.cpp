...
int nba97_game_match_clocks_from_match_tick(void* opaque,
    const Nba97MatchTickCall* call, Nba97GamePeriodValue* result) {
    auto* binding = static_cast<Nba97GameMatchClocksBinding*>(opaque);
    if (!binding || !call || result ||
        (call->pc != UINT32_C(0x80068d40) &&
         call->pc != UINT32_C(0x80068d58)) ||
        call->entry != UINT32_C(0x80067a60) || call->count != 1 ||
    ...
    ++binding->invocations;
    binding->result = nba97_game_match_clocks(&context, &binding->progress);
