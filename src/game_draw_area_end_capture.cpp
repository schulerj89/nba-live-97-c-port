...
namespace nba97 {
std::string captureGameDrawAreaEnd() {
 AreaEndComposed c;
 if(c.run()!=NBA97_TEXT_COMPLETE||!c.progress.completed||c.areas.size()!=2||c.packetRuns.size()!=2||c.copies!=4||c.submits!=2)
   throw std::runtime_error("draw-area end scene composition failed");
 check(c.get(0x80021f10u)==0xe4017c7fu&&c.get(0x80021f6cu)==0xe4017c7fu);
 const bool cacheEqual=std::memcmp(c.bytes.data()+c.at(0x800c55d0u),c.bytes.data()+c.at(0x80021eecu),0x5c)==0;check(cacheEqual);
 std::ostringstream o;o<<"{\"program\":\"GAMEONLY\",\"address\":\"0x8009A710\",\"inclusive_end\":\"0x8009A7DB\",\"bytes\":204,\"instructions\":51,"
  "\"classification\":\"no direct visual effect\",\"scope\":\"actual scene, display, draw, packet, draw-area end, video query, GPU command and BIOS trampoline; three synthetic packet-word helpers and submission/MMIO/BIOS services\","
  ...
  <<",\"cache_matches_last_environment\":"<<(cacheEqual?"true":"false")<<",\"commands\":["<<c.get(0x80021f10u)<<','<<c.get(0x80021f6cu)<<"],\"areas\":[";
