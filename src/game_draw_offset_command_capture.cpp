...
namespace nba97 {
std::string captureGameDrawOffsetCommand() {
 OffsetComposed c;
 if(c.run()!=NBA97_TEXT_COMPLETE||!c.progress.completed||c.offsets.size()!=2||c.dmaRuns.size()!=2||c.submissions.size()!=2||c.packetRuns.size()!=2||c.copies!=4||c.submits!=2)
   throw std::runtime_error("draw-offset scene composition failed");
 check(c.get(0x80021f14u)==0xe5001802u&&c.get(0x80021f70u)==0xe5001802u);
 ...
 std::ostringstream o;o<<"{\"program\":\"GAMEONLY\",\"address\":\"0x8009A7DC\",\"inclusive_end\":\"0x8009A823\",\"bytes\":72,\"instructions\":18,"
  "\"classification\":\"no direct visual effect\",\"scope\":\"actual scene/display/draw/packet/start/end/offset/video/GPU-command/BIOS-trampoline/graphics-submit/packet-DMA owners; two synthetic packet helpers, scheduler/critical services and mapped device registers without GPU consumption\","
  "\"completed\":true,\"parent_completed\":true,\"offset_calls\":"<<c.offsets.size()<<",\"packet_calls\":"<<c.packetRuns.size()<<",\"dma_calls\":"<<c.dmaRuns.size()<<",\"submit_calls\":"<<c.submits<<",\"copy_calls\":"<<c.copies
  <<",\"cache_matches_last_environment\":"<<(cacheEqual?"true":"false")<<",\"commands\":["<<c.get(0x80021f14u)<<','<<c.get(0x80021f70u)<<"],\"offsets\":[";
 for(size_t i=0;i<c.offsets.size();++i){if(i)o<<',';const auto&q=c.offsets[i];
