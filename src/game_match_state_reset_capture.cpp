...
bool GameMatchStateResetCapture::dispatch(const Nba97GameTextMemory* memory,const Nba97GameMatchInitializeEvent* event,Nba97GameMatchInitializeRegisters* registers) {
 ...
 if(nba97_game_match_state_reset_from_match_initialize(&binding,memory,event,registers)!=1||binding.result!=NBA97_TEXT_COMPLETE||!binding.progress.completed||binding.zero_invocations!=4||binding.roster_invocations!=1||fixture.calls.size()!=8||fixture.profile.receipt.empty())
  return false;
 ...
 "\"classification\":\"no direct visual effect\",\"scope\":\"actual initializer and match-state reset on the same retained memory, real zero-fill and roster owners; eight typed team/period services, no advancing native match\",\"completed\":true,\"same_parent_memory\":true,"
 ...
 o<<"],\"controller_profile_reset\":"<<fixture.profile.receipt<<"}";receipt=o.str();return true;
}
}
