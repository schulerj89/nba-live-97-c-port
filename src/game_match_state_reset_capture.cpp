...
bool GameMatchStateResetCapture::dispatch(const Nba97GameTextMemory* memory,const Nba97GameMatchInitializeEvent* event,Nba97GameMatchInitializeRegisters* registers) {
 ...
 "\"classification\":\"no direct visual effect\",\"scope\":\"actual initializer and match-state reset on the same retained memory, real zero-fill and roster owners; remaining typed controller/period services, no advancing native match\",\"completed\":true,\"same_parent_memory\":true,"
