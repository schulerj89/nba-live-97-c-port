...
struct Fixture {
 ...
 static int service(void* opaque,const Nba97GameTextMemory*,const Nba97GameMatchStateResetEvent* event,Nba97GameMatchStateResetMachine* machine) {
  auto&self=*static_cast<Fixture*>(opaque);self.calls.push_back(*event);
...
bool GameMatchStateResetCapture::dispatch(const Nba97GameTextMemory* memory,const Nba97GameMatchInitializeEvent* event,Nba97GameMatchInitializeRegisters* registers) {
 ...
 if(nba97_game_match_state_reset_from_match_initialize(&binding,memory,event,registers)!=1||binding.result!=NBA97_TEXT_COMPLETE||!binding.progress.completed||binding.zero_invocations!=4||binding.roster_invocations!=1||fixture.calls.size()!=9)
  return false;
 ...
 "\"classification\":\"no direct visual effect\",\"scope\":\"actual initializer and match-state reset on the same retained memory, real zero-fill and roster owners; nine typed input/team/period services, no advancing native match\",\"completed\":true,\"same_parent_memory\":true,"
