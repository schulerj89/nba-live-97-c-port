...
namespace nba97 {
std::string captureGameVideoMode() {
 VideoComposed c;
 if(c.run()!=NBA97_TEXT_COMPLETE||!c.progress.completed||c.videoBinding.completions!=2||c.copies!=2)
   throw std::runtime_error("video mode scene composition failed");
 check(c.commands==std::vector<U32>({0x0500500au,0x0503c064u,0x06cda328u,0x07048431u,0x0800002eu}));
 check(!c.binding.progress[0].screen_rectangle_changed&&c.binding.progress[1].screen_rectangle_changed);
 check(c.bytes[c.at(0x8002206eu)]==1&&c.bytes[c.at(0x800c563eu)]==1);
 std::ostringstream o;o<<"{\"program\":\"GAMEONLY\",\"address\":\"0x800985CC\",\"inclusive_end\":\"0x800985DB\",\"bytes\":16,\"instructions\":4,"
  "\"classification\":\"no direct visual effect\",\"scope\":\"actual scene, display environment, video query and BIOS trampoline; synthetic GPU and BIOS services\","
  "\"completed\":true,\"parent_completed\":true,\"scene_display_calls\":"<<c.binding.completions<<",\"copy_calls\":"<<c.copies<<",\"query_calls\":"<<c.videoBinding.completions
  <<",\"source_address\":2148291756,\"source_word_before\":1,\"source_word_after\":"<<unsigned(c.bytes[c.at(0x800c54acu)])<<",\"environment_video_byte\":"<<unsigned(c.bytes[c.at(0x8002206eu)])<<",\"commands\":[";
 for(size_t i=0;i<c.commands.size();++i){if(i)o<<',';o<<c.commands[i];}
 o<<"],\"queries\":[";
 for(unsigned i=0;i<2;++i){if(i)o<<',';const auto&q=c.videoBinding.progress[i];
   o<<"{\"call_pc\":"<<c.videoBinding.event[i].pc<<",\"operations\":"<<q.operations<<",\"reads\":"<<q.reads<<",\"return_v0\":"<<q.return_v0.word<<",\"return_mask\":"<<unsigned(q.return_v0.known_mask)<<",\"return_address\":"<<q.machine.registers.gpr[31].word<<"}";}
 o<<"]}";return o.str();
}
}
