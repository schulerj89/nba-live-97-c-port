...
namespace nba97 {
std::string captureGameGpuPacketDma() {
 PacketDmaComposed c;
 if(c.run()!=NBA97_TEXT_COMPLETE||!c.progress.completed||c.dmaRuns.size()!=2||c.submissions.size()!=2||c.packetRuns.size()!=2||c.copies!=4||c.submits!=2)
   throw std::runtime_error("GPU packet DMA scene composition failed");
 ...
 check(c.get(0x800c56b4u)==0x8009b1f8u&&c.get(0x800c56b8u)==0x80021f08u&&c.get(0x800c56bcu)==0);
 const bool cacheEqual=std::memcmp(c.bytes.data()+c.at(0x800c55d0u),c.bytes.data()+c.at(0x80021eecu),0x5c)==0;check(cacheEqual);
 std::ostringstream o;o<<"{\"program\":\"GAMEONLY\",\"address\":\"0x8009B1F8\",\"inclusive_end\":\"0x8009B243\",\"bytes\":76,\"instructions\":19,"
  "\"classification\":\"no direct visual effect\",\"scope\":\"actual scene/display/draw/packet/start/end/video/GPU-command/BIOS-trampoline/graphics-submit/packet-DMA owners; three synthetic packet helpers, scheduler/critical services and mapped device registers without GPU consumption\","
  "\"completed\":true,\"parent_completed\":true,\"dma_calls\":"<<c.dmaRuns.size()<<",\"submit_calls\":"<<c.submits<<",\"copy_calls\":"<<c.copies<<",\"cache_matches_last_environment\":"<<(cacheEqual?"true":"false")
  <<",\"port_addresses\":["<<0x1f801814u<<','<<0x1f8010a0u<<','<<0x1f8010a4u<<','<<0x1f8010a8u<<"],\"leaves\":[";
 for(size_t i=0;i<c.dmaRuns.size();++i){if(i)o<<',';const auto&q=c.dmaRuns[i];
   o<<"{\"operations\":"<<q.operations<<",\"reads\":"<<q.reads<<",\"stores\":"<<q.stores<<",\"return_v0\":"<<q.machine.registers.gpr[2].word<<",\"return_address\":"<<q.machine.registers.gpr[31].word<<",\"hi\":"<<q.machine.hi.word<<",\"port_words\":[";
   for(unsigned j=0;j<4;++j){if(j)o<<',';o<<c.writes[i][j];}o<<"]}";}
 o<<"]}";return o.str();
}
}
