...
extern "C" void nba97_game_draw_mode_command_packet_binding_init(
    Nba97GameDrawModeCommandPacketBinding *binding, size_t operation_budget,
...
extern "C" int nba97_game_draw_mode_command_from_packet(
  ...
  const bool target_entry = event->entry == UINT32_C(0x8009a5e8);
  ...
  binding->result = NBA97_TEXT_ARGUMENT;
  if (!target_kind || !target_entry || event->pc != UINT32_C(0x8009a3c8) ||
  ...
  binding->event = *event;
  binding->result =
      nba97_game_draw_mode_command(&context, &binding->progress);
