...
void copy_to_owner(const Nba97GameDrawPacketMachine &source,
                   Nba97GameDrawOffsetCommandMachine &destination) {
  for (unsigned index = 0u; index != 32u; ++index) {
...
void copy_from_owner(const Nba97GameDrawOffsetCommandMachine &source,
...
extern "C" void nba97_game_draw_offset_command_packet_binding_init(
    Nba97GameDrawOffsetCommandPacketBinding *binding, size_t operation_budget,
...
extern "C" int nba97_game_draw_offset_command_from_packet(
    void *opaque, const Nba97GameTextMemory *memory,
    const Nba97GameDrawPacketEvent *event,
    Nba97GameDrawPacketMachine *machine) {
  auto *binding =
      static_cast<Nba97GameDrawOffsetCommandPacketBinding *>(opaque);
  if (binding == nullptr || memory == nullptr || event == nullptr ||
      machine == nullptr)
    return 0;
  const bool target_kind = event->kind == NBA97_GAME_DRAW_PACKET_CHILD_8009A7DC;
  const bool target_entry = event->entry == UINT32_C(0x8009a7dc);
  ...
  binding->result = NBA97_TEXT_ARGUMENT;
  if (!target_kind || !target_entry || event->pc != UINT32_C(0x8009a3b0) ||
  ...
  binding->event = *event;
  binding->result =
      nba97_game_draw_offset_command(&context, &binding->progress);
