...
private:
    ...
    int captureTeamSelect() {
        ...
        frame("frontend-exit-drain-before");
        std::ofstream(output/"frontend_exit_drain_trace.json")<<nba97::captureFrontendExitDrain();
        frame("frontend-exit-drain-after");
        frame("frontend-clock-read-before");
        std::ofstream(output/"frontend_clock_read_trace.json")<<nba97::captureFrontendClockRead();
        frame("frontend-clock-read-after");
        ...
        frame("frontend-overlay-load-before");
        std::ofstream(output/"frontend_overlay_load_trace.json")<<nba97::captureFrontendOverlayLoad();
        frame("frontend-overlay-load-after");
        ...
        frame("frontend-resource-load-before");
        std::ofstream(output/"frontend_resource_load_trace.json")<<nba97::captureFrontendResourceLoad();
        frame("frontend-resource-load-after");
        frame("gameload-entry-before");
        std::ofstream(output/"gameload_entry_trace.json")<<nba97::captureGameloadEntry();
        frame("gameload-entry-after");
