...
void check(bool condition, const char *expression, int line) {
  ...
  throw std::runtime_error("camera frame transform native CPU fixture failed");
}
...
namespace nba97 {
std::string captureGameCameraFrameTransform() {
  Natural natural;
  CHECK(nba97_game_match_frame(&natural.frame, &natural.frame_progress) == NBA97_BODY_JOURNAL_LIMIT);
  const auto& p=natural.binding.progress;
  CHECK(natural.binding.invocations==1 && p.completed && natural.children==4);
  CHECK(natural.frame_progress.stopped_pc==0x800490c0);
  auto read=[&](uint32_t a) {uint32_t v=0;for(unsigned i=0;i<4;++i)v|=uint32_t(natural.ram[a-kBase+i])<<(8*i);return v;};
  CHECK(read(0x800f9fec)==104 && read(0x800f9ff0)==205 && read(0x800f9ff4)==306);
  std::ostringstream o;
  o << "{\"program\":\"GAMEONLY\",\"address\":\"0x80051098\",\"inclusive_end\":\"0x80051293\","
       "\"bytes\":508,\"instructions\":127,\"classification\":\"no direct visual effect\","
       "\"scope\":\"actual match frame; independent full machine; typed camera and GTE fixtures\","
       "\"completed\":true,\"translation_before\":[0,0,0],\"translation_after\":[104,205,306],"
       "\"operations\":" << p.operations << ",\"reads\":" << p.reads << ",\"stores\":" << p.stores
    << ",\"callbacks\":" << p.callbacks_completed << ",\"multiply_count\":" << p.multiply_count
    << ",\"frame_stack_pointer\":" << p.frame_stack_pointer << ",\"returned_sp\":" << p.machine.registers.gpr[29].word
    << ",\"restored_ra\":" << p.restored_return_address.word << ",\"next_pc\":" << natural.frame_progress.stopped_pc << "}";
  return o.str();
}
}
