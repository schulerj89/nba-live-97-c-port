...
namespace nba97 {
std::string captureGameDrawPacket() {
 PacketComposed c;
 if(c.run()!=NBA97_TEXT_COMPLETE||!c.progress.completed||c.packetRuns.size()!=2||c.copies!=4||c.submits!=2)
   throw std::runtime_error("draw packet scene composition failed");
 check(c.get(0x80021f08u)==0x09ffffffu&&c.get(0x80021f64u)==0x06ffffffu);
 check(c.get(0x80021f24u)==0x02332211u&&c.get(0x80021f28u)==0x00200040u&&c.get(0x80021f2cu)==0x00400040u);
 const bool cacheEqual=std::memcmp(c.bytes.data()+c.at(0x800c55d0u),c.bytes.data()+c.at(0x80021eecu),0x5c)==0;
 check(cacheEqual);
 std::ostringstream o;o<<"{\"program\":\"GAMEONLY\",\"address\":\"0x8009A344\",\"inclusive_end\":\"0x8009A5E7\",\"bytes\":676,\"instructions\":169,"
  "\"classification\":\"no direct visual effect\",\"scope\":\"actual scene, display, draw, packet, video query, GPU command and BIOS trampoline; five synthetic packet-word helpers and submission/MMIO/BIOS services\","
  "\"completed\":true,\"parent_completed\":true,\"packet_calls\":"<<c.packetRuns.size()<<",\"submit_calls\":"<<c.submits<<",\"copy_calls\":"<<c.copies
  <<",\"cache_matches_last_environment\":"<<(cacheEqual?"true":"false")<<",\"tags_after\":["<<c.get(0x80021f08u)<<','<<c.get(0x80021f64u)<<"],\"last_packet\":[";
 for(unsigned i=0;i<10;++i){if(i)o<<',';o<<c.get(0x80021f08u+i*4);}
 o<<"],\"packets\":[";
 for(size_t i=0;i<c.packetRuns.size();++i){if(i)o<<',';const auto&q=c.packetRuns[i];
   o<<"{\"operations\":"<<q.operations<<",\"reads\":"<<q.reads<<",\"stores\":"<<q.stores<<",\"callbacks\":"<<q.callbacks_completed<<",\"return_v0\":"<<q.machine.registers.gpr[2].word<<",\"return_address\":"<<q.machine.registers.gpr[31].word<<",\"hi\":"<<q.machine.hi.word<<"}";}
 o<<"]}";return o.str();
}
}
