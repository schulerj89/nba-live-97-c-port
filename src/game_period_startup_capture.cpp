...
struct Fixture {
    ...
    GamePeriodAudioFlagClearCapture audio_flag;
    GamePeriodAudioNoopCapture audio_noop;
    ...
    GameMatchBufferRecordCapture record;
    ...
    static int firstChild(void* user,const Nba97GameTextMemory* memory,const Nba97GameFirstPeriodStartupEvent* e,Nba97GameFirstPeriodStartupRegisters* r) {
        ...
        if(e->entry==0x8002a244u)return f.audio_flag.dispatch(memory,e,r);
        if(e->entry==0x8002a254u)return f.audio_noop.dispatch(memory,e,r);
    ...
    static int child(void* user,const Nba97GameTextMemory* m,const Nba97GamePeriodStartupEvent* e,Nba97GamePeriodStartupRegisters* r) {
        ...
        if(e->entry==0x80076b3cu)return f.record.dispatch(m,e,r);
...
static std::string capturePeriodFixture(int first_flag) {
    ...
    o<<",\"match_buffer_record\":"<<f.record.receipt();
    ...
    if(first_flag>=0) {
        ...
        o<<"],\"marker\":"<<f.get(0x800fdb94u,2)<<",\"presentation_halfword\":"<<f.get(0x800fdb4eu,2)
         <<",\"frame_stack_pointer\":"<<p.frame_stack_pointer<<",\"restored_ra\":"<<p.restored_return_address.word<<",\"audio_noop\":"<<f.audio_noop.receipt<<",\"audio_flag_clear\":"<<f.audio_flag.receipt<<",\"presentation_finish\":"<<f.presentation_finish.receipt<<",\"music_start\":"<<f.music.receipt<<",\"announcement\":"<<f.announcement.receipt<<"}";
