...
bool valid_machine(const Nba97GameActorContactGateMachine &machine) {
...
extern "C" void nba97_game_actor_contact_eligibility_binding_init(
...
extern "C" int nba97_game_actor_contact_eligibility_from_actor_contact_gate(
  ...
  Nba97GameActorContactEligibilityBinding *binding =
      static_cast<Nba97GameActorContactEligibilityBinding *>(user);
  ...
  if (event->pc != UINT32_C(0x8005facc) ||
  ...
  binding->result =
      nba97_game_actor_contact_eligibility(&context, &binding->progress);
  *machine = binding->progress.machine;
  return binding->result == NBA97_TEXT_COMPLETE ? 1 : 0;
}

extern "C" void nba97_game_actor_contact_eligibility_geometry_binding_init(
...
extern "C" int nba97_game_actor_contact_eligibility_geometry_child(
    void *user, const Nba97GameTextMemory *memory,
    const Nba97GameActorContactEligibilityEvent *event,
    Nba97GameActorContactEligibilityMachine *machine) {
  Nba97GameActorContactEligibilityGeometryBinding *binding =
      static_cast<Nba97GameActorContactEligibilityGeometryBinding *>(user);
  if (binding == nullptr || memory == nullptr || event == nullptr ||
      machine == nullptr)
    return 0;
  ...
  binding->result = NBA97_TEXT_ARGUMENT;
  const bool valid_pc =
      event->pc == UINT32_C(0x8005fa18) || event->pc == UINT32_C(0x8005fa70);
  if (!valid_pc || event->delay_slot_pc != event->pc + 4u ||
