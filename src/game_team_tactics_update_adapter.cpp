...
namespace {
bool valid_machine(const Nba97GameTeamTacticsMachine &machine) {
...
extern "C" int
nba97_game_team_tactics_update_from_match_tick(void *opaque,
                                               const Nba97MatchTickCall *call,
                                               Nba97GamePeriodValue *result) {
  auto *binding = static_cast<Nba97GameTeamTacticsBinding *>(opaque);
  if (binding == nullptr || call == nullptr || result != nullptr ||
      call->pc != UINT32_C(0x80068e28) || call->entry != UINT32_C(0x800747b0) ||
  ...
  context.user = binding->io_user;
  context.access_journal = binding->access_journal;
  context.access_journal_capacity = binding->access_journal_capacity;
  ++binding->invocations;
  binding->event = *call;
  binding->result =
      nba97_game_team_tactics_update(&context, &binding->progress);
