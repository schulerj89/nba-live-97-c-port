...
int nba97_game_match_service_publish_from_match_tick(void* opaque,
    const Nba97MatchTickCall* call, Nba97GamePeriodValue* result) {
    auto* binding =
        static_cast<Nba97GameMatchServicePublishBinding*>(opaque);
    if (!binding || !call || result ||
        call->pc != UINT32_C(0x80068d7c) ||
        call->entry != UINT32_C(0x8002de34) || call->count != 0 ||
    ...
    ++binding->invocations;
    binding->result =
        nba97_game_match_service_publish(&context, &binding->progress);
