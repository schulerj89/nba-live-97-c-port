...
void checkAt(bool value,unsigned line) {
    ...
    if(!value) {
        std::fprintf(stderr,
            "game CD-sync-callback check %u failed at line %u\n",checks,line);
        std::exit(1);
    }
}
#define check(value) checkAt((value),__LINE__)

constexpr std::uint32_t Slot=0x800c57e8u;
constexpr std::uint32_t SourceDefault=0x8009da04u;

struct Fixture {
    std::array<std::uint8_t,12> bytes{},known{};
    Nba97GameTextRegion region{Slot-4u,bytes.data(),known.data(),bytes.size()};
    Nba97GameCdSyncCallbackContext context{{&region,1},10,0};
    Nba97GameCdSyncCallbackProgress progress{};
    ...
    int run() {
        return nba97_game_cd_sync_callback(&context,&progress);
    }
};
...
void prefixes_and_validation() {
    ...
    check(before_read.run()==NBA97_TEXT_LIMIT && !before_read.progress.completed &&
        ...
        before_read.progress.stopped_pc==0x8009dbfcu &&
    ...
    check(after_read.run()==NBA97_TEXT_LIMIT &&
        ...
        after_read.progress.stopped_pc==0x8009dc04u &&
    ...
    check(missing.run()==NBA97_TEXT_RESOURCE &&
        ...
        missing.progress.stopped_pc==0x8009dbfcu &&
    ...
    Nba97GameCdSyncCallbackProgress progress{};
    check(nba97_game_cd_sync_callback(nullptr,&progress)==NBA97_TEXT_ARGUMENT);
    Fixture null_out;
    check(nba97_game_cd_sync_callback(&null_out.context,nullptr)==
        NBA97_TEXT_ARGUMENT);
}
}
...
int main() {
    ...
    std::printf("game_cd_sync_callback: %u checks passed\n",checks);
}
