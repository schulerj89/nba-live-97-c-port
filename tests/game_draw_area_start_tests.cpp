...
void set_word(Nba97GameDrawAreaStartWord &word, uint32_t value,
              uint8_t known = 15u) {
  word.word = value;
...
struct Fixture {
  ...
  Nba97GameTextRegion region{kGlobals, bytes.data(), known.data(),
                             bytes.size()};
  std::array<Nba97GameDrawAreaStartAccess, 4> journal{};
  Nba97GameDrawAreaStartContext context{};
  ...
  int run() { return nba97_game_draw_area_start(&context, &progress); }
};
...
uint32_t expected_command(int32_t x, int32_t y, uint8_t type) {
  ...
  return UINT32_C(0xe3000000) | ((static_cast<uint32_t>(y) & mask) << bits) |
...
void partial_knownness_and_branch_prefixes() {
  ...
  CHECK(x_sign.progress.stopped_pc == UINT32_C(0x8009a64c));
  ...
  CHECK(y_sign.progress.stopped_pc == UINT32_C(0x8009a690));
  ...
  CHECK(x_limit_sign.progress.stopped_pc == UINT32_C(0x8009a674));
  ...
  CHECK(y_limit_sign.progress.stopped_pc == UINT32_C(0x8009a6b8));
  ...
  CHECK(type.progress.stopped_pc == UINT32_C(0x8009a6e0));
  ...
  CHECK(low_partial.progress.stopped_pc == UINT32_C(0x8009a674));
}
...
void skipped_reads_failures_budgets_and_atomicity() {
  ...
  CHECK(negative.run() == NBA97_TEXT_COMPLETE);
  CHECK(negative.progress.operations == 1u);
  CHECK(negative.progress.return_v0.word == UINT32_C(0xe3000000));

  Fixture missing_x;
  missing_x.context.memory.count = 0u;
  CHECK(missing_x.run() == NBA97_TEXT_RESOURCE);
  CHECK(missing_x.progress.stopped_pc == UINT32_C(0x8009a65c));
  ...
  CHECK(missing_y.progress.stopped_pc == UINT32_C(0x8009a6a0));
  ...
  CHECK(missing_type.progress.stopped_pc == UINT32_C(0x8009a6d0));
  ...
  for (size_t malformed = 0u; malformed != 8u; ++malformed) {
    ...
    if (malformed == 4u || malformed == 5u) {
      CHECK(fixture.progress.stopped_pc == UINT32_C(0x8009a65c));
      CHECK(fixture.progress.return_v0.word == kGlobals + 4u);
    } else if (malformed == 6u || malformed == 7u) {
      CHECK(fixture.progress.stopped_pc == UINT32_C(0x8009a6a0));
      CHECK(fixture.progress.return_v0.word == kGlobals + 6u);
    } else {
      CHECK(fixture.progress.stopped_pc == UINT32_C(0x8009a6d0));
...
void unknown_ra_mapping_journal_and_determinism() {
  ...
  CHECK(unknown_ra.progress.stopped_pc == UINT32_C(0x8009a708));
  ...
  CHECK(truncated.journal[0].pc == UINT32_C(0x8009a65c));
  ...
  Fixture nulls;
  CHECK(nba97_game_draw_area_start(nullptr, &nulls.progress) ==
        NBA97_TEXT_ARGUMENT);
  CHECK(nba97_game_draw_area_start(&nulls.context, nullptr) ==
        NBA97_TEXT_ARGUMENT);
...
int main() {
  ...
  std::printf("game_draw_area_start_tests: %zu checks passed\n", checks);
  return 0;
}
