...
void checkAt(bool value,unsigned line) {
    ++checks;
    if(!value) {
        std::fprintf(stderr,"game VBlank-shutdown check %u failed at line %u\n",
            checks,line);
        std::exit(1);
    }
}
...
constexpr std::uint32_t CallerRa=0x80029b6cu;
constexpr std::uint32_t IncomingFp=0xf6f6f6f6u;
...
struct Fixture {
    ...
    Nba97GameVblankShutdownContext context{{&region,1},10,EntrySp,
        CallerRa,IncomingFp,Gp,io,this};
    Nba97GameVblankShutdownProgress progress{};
    std::vector<Nba97GameVblankShutdownEvent> calls;
    Nba97GameVblankShutdownValue child{Handler,1};
    bool refuse=false;
    ...
    int run() {return nba97_game_vblank_shutdown(&context,&progress);}
};
...
void removes_handler_and_restores_frame() {
    ...
    check(f.calls.size()==1 &&
        f.calls[0].kind==NBA97_GAME_VBLANK_SHUTDOWN_INTERRUPT_CALLBACK &&
        f.calls[0].pc==0x800a44ecu && f.calls[0].entry==0x8009860cu &&
        f.calls[0].argument_count==2 && f.calls[0].argument[0]==0 &&
        f.calls[0].argument[1]==0 && f.calls[0].stack_pointer==FrameSp &&
        f.calls[0].frame_pointer==FrameSp &&
        f.calls[0].global_pointer==Gp &&
        f.calls[0].return_address==0x800a44f4u);
...
void preserves_live_child_and_epilogue_values() {
    ...
    check(unknown_ra.run()==NBA97_TEXT_UNKNOWN &&
        ...
        unknown_ra.progress.stopped_pc==0x800a44f8u &&
    ...
    check(unknown_fp.run()==NBA97_TEXT_UNKNOWN &&
        ...
        unknown_fp.progress.stopped_pc==0x800a44fcu &&
...
void refusals_limits_and_validation() {
    ...
    check(no_io.run()==NBA97_TEXT_IO_REFUSED &&
        ...
        no_io.progress.stopped_pc==0x800a44ecu &&
    ...
    static constexpr std::uint32_t pcs[5]={0x800a44d8u,0x800a44dcu,
        0x800a44ecu,0x800a44f8u,0x800a44fcu};
    ...
    Nba97GameVblankShutdownProgress progress{};
    check(nba97_game_vblank_shutdown(nullptr,&progress)==NBA97_TEXT_ARGUMENT);
    Fixture null_out;
    check(nba97_game_vblank_shutdown(&null_out.context,nullptr)==
        NBA97_TEXT_ARGUMENT);
}
}
...
int main() {
    ...
    std::printf("game_vblank_shutdown: %u checks passed\n",checks);
}
