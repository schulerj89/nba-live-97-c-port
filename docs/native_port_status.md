...
| CD-ready callback exchange | Complete six-instruction PsyQ `CdReadyCallback` owner `9DBE0` is composed at natural main caller `29B3C`. It returns source default callback `9D9DC`, clears slot `C57E4`, and records exact state plus pixel-identical before/after frames. Raw unchecked replacements, read-before-store order, unknown prior return values, and unconditional storage remain. | This updates retained PS1 callback state only; it does not register or invoke a host callback. Earlier `CdInit` remains a typed boundary, and no CD-device or gameplay effect is claimed. See [CD-ready callback exchange](game_cd_ready_callback_workflow.md). |
| CD-sync callback exchange | Complete six-instruction PsyQ `CdSyncCallback` owner `9DBF8` is composed at natural main caller `29B44`. It returns source default callback `9DA04`, clears slot `C57E8`, and records exact state plus pixel-identical before/after frames. Raw unchecked replacements, read-before-store order, unknown prior return values, and unconditional storage remain. | This is retained PS1 command-completion callback state, not a Windows callback or native CD backend. Earlier `CdInit` and internal `CD_sync` remain typed boundaries, and no gameplay effect is claimed. See [CD-sync callback exchange](game_cd_sync_callback_workflow.md). |
| VBlank shutdown wrapper | Complete 14-instruction owner `A44D4` is composed at its only caller, main `29B64`, after the second twenty-presentation wait. It calls `InterruptCallback(0,NULL)`, removes retained handler `A450C`, preserves live child `v0` and mutable `ra`/`s8` reloads, and emits exact logs plus pixel-identical frames. | `9860C` remains an explicit PsyQ callback-table boundary. This changes retained PS1 interrupt state only; it does not unregister a Windows callback or change host timing. See [VBlank shutdown](game_vblank_shutdown_workflow.md). |
| Game-clock shutdown wrapper | Complete 14-instruction owner `9167C` is composed at main `29B6C`, immediately after VBlank shutdown. It calls `InterruptCallback(6,NULL)`, removes retained Timer 2 handler `916B4` from slot `C54E8`, preserves live child `v0` and mutable `ra`/`s8` reloads, and emits exact logs plus pixel-identical frames. | `9860C` remains an explicit PsyQ callback-table boundary. This changes retained PS1 interrupt state only; it neither stops a host timer nor unregisters a Windows callback. See [game-clock shutdown](game_clock_shutdown_workflow.md). |
| Controller-suspend wrapper | Complete 14-instruction owner `8F19C` is composed at its only caller, main `29B74`, immediately after clock shutdown. Active flag zero calls `91224` once and stores one at `C4A70`; arbitrary nonzero flags skip both and return unchanged. Exact logs and pixel-identical frames preserve pre-frame load, delay-slot spill, discarded child `v0` and live `ra`. | `91224` remains a typed PS1 shutdown-service boundary. This changes retained input state only; it does not detach or suppress native keyboard/gamepad input. See [controller suspend](game_controller_suspend_workflow.md). |
| Shutdown-table zero fill | Zero entry `A3A74` plus its 80-instruction fallthrough core `A3A78..A3BB7` is composed at main `29B84`. The natural 32-byte clear at `D6DEC` emits the exact nine stores/36 traffic bytes and pixel-identical frames while preserving SWR/SWL overlap, signed-count bugs, 32-bit wrap and live `v0`. | The table is already zero after the recovered clock initializer, so this is an idempotent retained-state operation, not visual gameplay. Other zero/general-fill callers remain separate. See [shutdown-table zero fill](game_memory_zero_workflow.md). |
| FELOAD optimized memory copy | Complete 200-instruction `AA468` owner is composed at main `29B94`. It copies the loader-owned 5,136-byte fixture from `80123400` to `801E0000` with 2,568 exact accesses, after which main reads copied entry `801E0100`. Full forward/backward, aligned/unaligned, overlap, knownness, trap, bounded-runaway and prefix behavior is source-compared; byte snapshots change while frames remain pixel-identical. | The payload is a deterministic diagnostic fixture supplied by the still-typed `941C8` service, not retail FELOAD or a production filesystem load. Other `AA468` callers are not automatically composed, and this CPU move does not render gameplay. See [FELOAD memory copy](game_memory_copy_workflow.md). |
