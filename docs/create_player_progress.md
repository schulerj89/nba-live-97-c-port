...
The camera audit recovered base rotation `{1500,0,0}`, translation `{0,-384,3328}`, geometry offset `{256,120}`, projection distance `H=160`, and frontend angles `{2051,191,0}`. Live RAM then supplied both field-dependent states: appearance fields use clip 0 with root `{132,-78,1056}`, yaw `0x3F0`, and parent translation `{162,104,231}`; all other fields use clip 1 with root `{256,0,640}`, yaw `808`, and parent translation `{544,9,594}`. `FUN_80069098` resolves to three roots and the static parent table `[-1,0,1,2,-1,4,5,6,-1,8,9,10,9,12,13,14,9,16,17,18]`. The runtime applies all 20 local rotations, `height * 0x270` 16.16 scale, signed root lift, and root yaw. Synchronized checks cover the full transform/projection path, all 753 SXY outputs, all 251 descriptor-0 ordering keys, and the live OT packet sequence. The earlier `{398,94}` face-219 read was capture skew; the synchronized retail result and port are both `{396,94}` at depth 614. Remaining model work is confined to pixel-level PS1 triangle coverage/interpolation and whole-screen visual scoring, not camera, vertex, packet, material, or VRAM reconstruction. These are audit counts, not a whole-feature percentage.
...
The verifier builds the app; runs the transaction/editor, model decoder, VRAM addressing, mocap, fixed-point transform, GTE composition, hierarchy, and camera suites; and checks the recovered `NCLIP`/ordering/texture totals. It captures manager/editor states and all three Delete contexts twice, validates all sixteen 512x240 outputs, requires identical SHA-256 hashes between runs, and separately proves selector, mocap, and scroll phases change pixels. An opt-in `NBA97_CREATE_ORIGINAL_VRAM` audit compares sampled words, indices, and CLUT entries directly with a 1 MiB no$psx VRAM dump. Evidence and private assets remain local only.
