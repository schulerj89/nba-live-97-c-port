...
The primary manager/editor inventory contains 1,020 instructions across 14 functions. The input slice now additionally accounts for the recovered 13-byte name buffers and 20/6-vblank selector/list transitions. Nine shared model helpers (1,029 instructions) are separately inventoried: their complete D/E/F/S asset family, relinked anchor graph, 81 source-triangle banks, 251 surface descriptors, exact private trig table, base fixed-point transform path, recovered `FUN_80069098` hierarchy, and the 11-instruction `FUN_8006734C` RTPS wrapper are native-exercised. Seven are now exercised by native code; two remain research-only.

The camera audit recovered rotation `{1500,0,0}`, translation `{0,-384,3328}`, geometry offset `{256,120}`, and projection distance `H=160` (FEONLY static `0x800C6508`). The port's `{128,0}` viewport composition remains explicit and is not counted as a recovered GTE register. `FUN_80069098` pointer assignments resolve to three roots and the exact static parent table `[-1,0,1,2,-1,4,5,6,-1,8,9,10,9,12,13,14,9,16,17,18]`. The hierarchy expands the RTPS body bounds from roughly 8x12 to 16x25 pixels with all 753 decoded corners visible and no saturation. That is a real upstream improvement, but it also proves static hierarchy alone is insufficient for the original on-screen scale. The next isolated boundary is exact ZFEMOCAP joint rotation/root placement; the visible preview retains the compatibility camera until that is recovered. Secondary head construction and textured GPU-packet equivalence also remain research/translation work. These are audit counts, not a whole-feature percentage.
...
The verifier builds the app, runs the recovered C transaction/editor behavior tests plus the model decoder, fixed-point transform, hierarchy, and RTPS/camera numeric suites. Its private-asset hierarchy smoke draws exact one-pixel joint/corner output plus a readable diagnostic inset to `.local/verification/create_player/hierarchy/smoke.ppm`. It also captures manager/editor states and all three Delete contexts twice, validates all sixteen 512x240 outputs, requires identical SHA-256 hashes between runs, and separately proves the recovered selector, mocap, and scroll phases change pixels. Evidence remains local only.
